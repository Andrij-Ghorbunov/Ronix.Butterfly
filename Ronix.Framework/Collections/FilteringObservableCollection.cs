...
{
    /// <summary>
    /// Extends <see cref="FilteredObservableCollection{T}"/> functionality by observing property updates of each element.
    /// </summary>
    /// <typeparam name="T">Type of items in the collection. Has to implement <see cref="INotifyPropertyChanged"/> interface.</typeparam>
    public class FilteringObservableCollection<T> : FilteredObservableCollection<T> where T : INotifyPropertyChanged
    {
        ...
        /// <summary>
        /// Initializes a new instance of <see cref="FilteringObservableCollection{T}"/>, based on a certain collection.
        /// </summary>
        ...
        public FilteringObservableCollection(ObservableCollection<T> source, Func<T, bool> filter = null, Expression<Func<T>> property = null)
            : base(source, filter)
        {
            TryInit();
            if (property != null)
        ...
        private void ItemPropertyChanged(object sender, PropertyChangedEventArgs e)
        {
            ...
            if (contains && !filter)
                ...
                Insert(GetIndex(item), item);
        }

        private int GetIndex(T item)
        {
            return Source.Take(Source.IndexOf(item)).Count(it => Filter(it));
        }
